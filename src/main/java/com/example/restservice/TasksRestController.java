...
public class TasksRestController {
    ...
    @PostMapping
    public ResponseEntity<?> handleGetNewTasks(
            @RequestBody NewTaskPayload payload,
            UriComponentsBuilder uriComponentsBuilder,
            Locale locale) {
        if (payload.details() == null || payload.details().isBlank()) {
            final var massage = this.messageSource.getMessage("task.create.details.errors",
                    new Object[0],locale);
            return ResponseEntity.badRequest()
                    .contentType(MediaType.APPLICATION_JSON)
                    .body(new ErrorPresentation(List.of(massage)));
        } else {
