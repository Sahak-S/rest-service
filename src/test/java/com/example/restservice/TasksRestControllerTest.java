...
class TasksRestControllerTest {
    ...
    void handleGetAllTasks_ReturnValidEntity() {

//        given
        var tasks = List.of(new Task(UUID.randomUUID(), "arajin ashxatanq", false),
                new Task(UUID.randomUUID(), "erkrord ashxatanq", true));
...
//        var user = new ApplicationUser(UUID.randomUUID(), "user1", "password1");
        var details = "errord ashxatanq";

        // when
        var responseEntity = this.controller.handleGetNewTasks(new NewTaskPayload(details),
...
//        var user = new ApplicationUser(UUID.randomUUID(), "user1", "password1");
        ...
        doReturn(errorMessage).when(this.messageSource)
                .getMessage("task.create.details.errors", new Object[0], locale);
